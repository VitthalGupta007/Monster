...
namespace VXMonster.Core.Easing
{
    ...
    {
        public static EasingJobAnimation DoPositionJob(this Transform transform, Transform targetTransform, float duration, float delay = 0, bool useUnscaledTime = false, EasingType easingType = EasingType.Linear)
        {
            return new PositionEasingJobAnimation(transform, targetTransform, duration, delay, useUnscaledTime, easingType);
        }
